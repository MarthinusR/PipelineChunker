                ...
                yield return Chunk<Pipeline, int, int>(
                    ...
                    ConduitInitializer: (pipeline) => {
                        try {
                            ...
                            Debug.WriteLine($"{typeof(MainConduit).FullName} Caught exception-1:{Environment.NewLine}{e}");
                        }
                        return 0;//NOP
                    },
                    ChunkTransform: static (channel, pipeline, values, pristineTotal) => {
                        try {
                            Debug.WriteLine($"MainConduit[?] - ChunkTransform -- Flush");
                            pipeline.Flush();
                            for (int i = 0; i < pristineTotal; i++) {
                            ...
                            Debug.WriteLine($"{typeof(MainConduit).FullName} Caught exception-2:{Environment.NewLine}{e}");
                        }
                        return values;
                    },
                    ConduitOperation: (dt, pair, e, communicator) => {
                        if (e != null) {
                            Debug.WriteLine($"{typeof(MainConduit).FullName} Caught exception-3:{Environment.NewLine}{e}");
                            communicator.ExceptionHandled = true;
                        }
                        Debug.WriteLine($"{typeof(MainConduit).FullName} Caught exception-4:{Environment.NewLine}{pair.Conduit}");
        ...
        private class OtherConduit : Pipeline.Conduit<OtherConduit> {
            ...
            public override IEnumerator<OtherConduit> GetEnumerator() {
                ...
                yield return Chunk<int, int, int>(
                    ...
                    ChunkTransform: static (channel, start, values, pristineTotal) => {
                        try {
                            for (int i = 0; i < pristineTotal; i++) {
                            ...
                            Debug.WriteLine($"{typeof(OtherConduit).FullName} Caught exception-6:{Environment.NewLine}{e}");
                        }
                        return values;
                    },
                    ConduitOperation: (start, pair, e, communicator) => {
                        if(e != null) {
                            Debug.WriteLine($"{typeof(OtherConduit).FullName} Caught exception-7:{Environment.NewLine}{e}");
                            communicator.ExceptionHandled = true;
                        }
                        if(CallingConduitId == 4)
                            throw new Exception($"Debug Test 4 - ConduitOperation | CallingConduitId:{CallingConduitId}");
